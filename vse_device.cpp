...
VseDevice::VseDevice(VseWindow &window) : window{window} {
  createInstance();
  setupDebugMessenger();
  createSurface();
  pickPhysicalDevice();
...
SwapChainSupportDetails VseDevice::querySwapChainSupport(
...
VkFormat VseDevice::findSupportedFormat(const std::vector<VkFormat> &candidates,
...
uint32_t VseDevice::findMemoryType(uint32_t typeFilter,
...
void VseDevice::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage,
...
void VseDevice::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                           VkDeviceSize size) {
...
void VseDevice::copyBufferToImage(VkBuffer buffer, VkImage image,
...
void VseDevice::createImageWithInfo(const VkImageCreateInfo &imageInfo,
                                    VkMemoryPropertyFlags properties,
                                    VkImage &image,
